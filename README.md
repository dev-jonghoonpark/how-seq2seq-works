...
| 절 | 주제 | 인터랙티브 데모 |
|---|---|---|
| §1 | 인코더–디코더 구조 | 번역 과정 스텝별 애니메이션 (은닉 상태 → 문맥 벡터 c → 자기회귀 생성) · 다층 LSTM 구조도 (1/2/4층 전환, 층별 (h, C)가 디코더 같은 층으로 넘어가는 모습) |
| §2 | 고정 길이 병목 | 문장 길이 압축 시각화 · 고정 벡터 모델의 문장 길이 vs BLEU 차트 (Bahdanau 2015 재구성) |
...
| §6 | 유산 | 병목 → 어텐션 → 트랜스포머로 이어지는 계보 · 고정 벡터 vs 어텐션 BLEU 비교 차트 + 이해도 퀴즈 6문제 |
